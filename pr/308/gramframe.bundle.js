(function() {
  ...
  function createSymbolMark(symbolType, cx, cy, size, color) {
    ...
    switch (resolved) {
      case "square": {
        el = document.createElementNS(SVG_NS$5, "rect");
      ...
      case "diamond": {
        el = document.createElementNS(SVG_NS$5, "polygon");
      ...
      case "circle":
      default: {
        el = document.createElementNS(SVG_NS$5, "circle");
  ...
  function plateLabel(text, options = {}) {
    ...
    const plate = document.createElementNS(SVG_NS$4, "rect");
  ...
  function fitShapes() {
    return [
      ...
      const path = document.createElementNS(SVG_NS$3, "path");
  ...
  function rect(x, y, width, height, radius) {
    const shape = document.createElementNS(SVG_NS$3, "rect");
  ...
  function createIcon(name) {
    ...
    const svg = (
      /** @type {SVGSVGElement} */
      document.createElementNS(SVG_NS$3, "svg")
    );
  ...
  function aspectLockedRect(viewport, bounds, start, current) {
    const { renderWidth, renderHeight } = getRenderDimensions(viewport);
  ...
  function createRegionOverlay() {
    ...
    const dim = document.createElementNS(SVG_NS$2, "path");
    dim.setAttribute("class", "gram-frame-region-dim");
    dim.setAttribute("fill-rule", "evenodd");
    overlay.appendChild(dim);
    const box = document.createElementNS(SVG_NS$2, "rect");
  ...
  function renderRegionOverlay(overlay, view) {
    ...
    const inner = `M${rect2.x} ${rect2.y}H${rect2.x + rect2.width}V${rect2.y + rect2.height}H${rect2.x}Z`;
    overlay.children[0].setAttribute("d", `${outer}${inner}`);
    const box = overlay.children[1];
    box.setAttribute("x", String(rect2.x));
    box.setAttribute("y", String(rect2.y));
    box.setAttribute("width", String(rect2.width));
    box.setAttribute("height", String(rect2.height));
    while (overlay.children.length > 2 && overlay.lastChild) {
  ...
  function drawSelection(instance, session) {
    ...
    const rect2 = aspectLockedRect(viewportOf(instance), bounds, session.start, session.current);
    const { freqSpan, timeSpan } = regionOf(instance, rect2);
    renderRegionOverlay(session.overlay, { rect: rect2, bounds, freqSpan, timeSpan });
  }
  ...
  function finishSelection(instance, session) {
    const rect2 = aspectLockedRect(viewportOf(instance), boundsFor(instance), session.start, session.current);
