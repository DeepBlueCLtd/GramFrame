(function() {
  ...
  function computeAvailableRenderSize(instance) {
    ...
    const chromeHeight = [".gram-frame-transport", ".gram-frame-display-bar"].map((selector) => cell.querySelector(selector)).reduce((total, element) => total + (element instanceof HTMLElement ? element.offsetHeight + (parseFloat(window.getComputedStyle(element).marginTop) || 0) : 0), 0);
  ...
  function mountDisplayRangeBar(instance, display) {
    const cell = instance.ui.mainCell;
