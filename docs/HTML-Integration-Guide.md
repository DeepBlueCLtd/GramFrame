...
| `colour-map` | `colour` / `grey` / `inferno` | `colour` | Paint with the colour table; as grey shades (black quietest, white loudest) as a legacy renderer draws; or with inferno, a perceptually uniform map whose brightness rises strictly with level. The selector on the transport bar changes the same choice live, without re-analysing |
