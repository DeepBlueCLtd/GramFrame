...
| D5 | Colour: log power, floor at the 5th percentile and ceiling at the 99.9th percentile of the file, 256-entry dark blue → blue → yellow → orange → red LUT (chosen against `sample/mock-gram.png`). *Corrected by spec 171 FR-002: this row said blue→cyan→yellow→white, which is not the table `audio/colourMap.js` shipped.* | contracts/audio-pipeline.md |
...
| ~~D10~~ | **Removed by spec 171 FR-003.** The reveal rule is withdrawn: `viewTop` is clamped by the duration, the image clip is the axes area, and `isTimeRevealed` no longer exists | contracts/player-api.md |
...
| D16 | Rate change preserves pitch, assigned explicitly on the element and selectable per exercise with the `preserve-pitch` config row (*corrected and superseded by spec 171 FR-001/FR-021/FR-022; this row said the browser default was used*); readouts are unaffected because the gram is never re-analysed (FR-022) | — |
