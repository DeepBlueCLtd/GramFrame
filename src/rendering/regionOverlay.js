 ...
 *
 * Draws the rubber-band rectangle, the dimmed surround that makes the selection
 ...
 * `core/regionZoom.js` and arrives here already aspect-locked and clamped, so
 * this file holds no idea of what a drag is.
 */
...
export function renderRegionOverlay(overlay, view) {
  ...
  // the surround dims in a single element and the selection stays clear.
  const outer = `M${bounds.left} ${bounds.top}H${bounds.right}V${bounds.bottom}H${bounds.left}Z`
  const inner = `M${rect.x} ${rect.y}H${rect.x + rect.width}V${rect.y + rect.height}H${rect.x}Z`
...
/**
 * The span readout, placed above the box — or below it when the box is hard
 * against the top edge of the gram.
